...
computing resources.
Fortunately, NYU provides access to high performance computing
(HPC) for affiliated researchers.
Unfortunately, HPC is surprisingly challenging to use.
...
the information I wish I was told years ago.

Note: There are many other resources describing how you can access HPC. 
The official HPC docs is [here](https://sites.google.com/nyu.edu/nyu-hpc/accessing-hpc?authuser=0).
I just found all the resources I could find difficult to follow or confusing.

### **Initial Set Up**

There are several steps to getting an HPC account and authenticating into it.

1. **Requesting access to HPC**. The HPC cluster is a powerful resource.
As a result, NYU restricts who can use it. If you're not a NYU faculty,
...
2. **Setting up the VPN (Optional)**. Accessing HPC requires
...
3. **Authenticating to HPC**. There are several ways you can authenticate from
your personal computer to HPC. I strongly recommend using VS Code.
...
<center><img src="images/hpc.png" width="600"></center>

Now that you're able to access it, let's understand how HPC works.
You will access HPC from a personal computer (on NYU Wi-Fi or a VPN).
...
(In your request, you'll specify how long you want the login node and how many CPUs/GPUs you'll need.)
Once you're on a compute node, you will complete the following three steps:
...
It's sadly a pain to do; all the instructions are in the last section.
Before we get there, let's understand the folder system.

HPC allocates two folders for every user. As far as I can tell, one of them
is basically useless (of course, this is where they initialize each session).
If your NetID is `abc123`, then the useless folder is called `home/abc123`.
The home folder has limited space and, as I've found out from even
more angry emails, should **not** be used for storing code.
The scratch folder, appropriately called `scratch/abc123` has lots
of space and should be used for storing code.

### **Running Code**
There are two ways to run code on HPC: interactively
or in a sbatch job.
Both methods begin in a login node.
Before you try either of the methods described in this section, 
you **must** have completed the instructions 
...
Once you've been assigned a compute node (this may take a bit), 
...
The `nv` flag tells the singularity to expect a GPU (this is fine in general
...
#### **Sbatch Job**
Many jobs take more time to run. We can submit them using slurm.
...
Here, we load a singularity, load the conda environment, activate the conda environment
`envname` and then run the code in `code.py`.
If the sbatch file is called `run.slurm`, you can submit it by running `sbatch run.slurm`.
You can check the status of the job by running `squeue --user abc123`.
In addition,
errors and outputs will appear in the respective files once the job has been
assigned to a node.

### **One Time Set Up for Singularity and Conda**

I struggled to find error-free instructions for setting 
up the singularity and conda environments for a long time.
Fortunately,
[Lucas Rosenblatt](https://www.lucasrosenblatt.com/) kindly shared the following
instructions and I found they worked!

After authenticating to HPC and connecting to a login node, you should
...
We chose one of the configurations with a decent amount of memory.
Now you'll unzip your overlay:

``gunzip -vvv ./overlay-25GB-500K.ext3.gz``

Don't be surprised if the unzipping process
takes a little while to run on the login node.
...
(use the code snippet from before).
...
Finally, you can install all your by running `pip install packagename`.
...
Fortunately, we can handle this with a batch script.
Make sure you're in a singularity on a compute node and 
in the `/ext3/` folder.
From here, run `bash` and then:
...
For sanity, double check that you see a folder called `miniconda3` in
`/ext3/` as well.
